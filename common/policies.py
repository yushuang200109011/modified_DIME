...
class BaseJaxPolicy(BasePolicy):
    ...
    def prepare_obs(self, observation: Union[np.ndarray, Dict[str, np.ndarray]]) -> Tuple[np.ndarray, bool]:
        ...
        if isinstance(observation, dict):
            ...
            observation = np.concatenate(
                [np.atleast_2d(observation[key].reshape(-1, *self.observation_space[key].shape)) for key in keys],
                axis=1,
            )
    ...
    def predict2(
        ...
        all_actions = self._predict2(observation, deterministic=deterministic)
        latent_actions = all_actions[1]
        ...
        return actions, latent_actions, state
