    ...
    def _log_success_callback(self, locals_: Dict[str, Any], globals_: Dict[str, Any]) -> None:
        ...
        if locals_["done"]:
            ...
            if len(local_per_time_step_buffer) > 0:
                local_per_time_step_buffer = np.array(local_per_time_step_buffer)
                local_per_time_step_buffer = np.mean(local_per_time_step_buffer, axis=0)
                self._per_time_is_success_buffer.append(1 if local_per_time_step_buffer > 0 else 0)
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and (self.n_calls % self.eval_freq == 0 or self.n_calls == 1):
            ...
            if len(self._is_success_buffer) > 0:
                ...
                per_time_success_rate = np.mean(self._per_time_is_success_buffer)
                if self.verbose >= 1:
                    print(f"Success rate: {100 * success_rate:.2f}%")
                self.logger.record("eval/success_rate", success_rate)
                self.logger.record("eval/per_time_success_rate", per_time_success_rate)
